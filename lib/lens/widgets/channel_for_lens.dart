...
class ChannelForLens extends StatefulWidget {
  const ChannelForLens({super.key});

  @override
  State<ChannelForLens> createState() => _ChannelForLensState();
}

class _ChannelForLensState extends PaginationListenerState<ChannelForLens> {
  @override
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ChannelForLensCubit, ChannelForLensState>(
  ...
  Future<void> onFetched() async {
    if (mounted) {
      await context.read<ChannelForLensCubit>().fetch();
    }
  }
}
...
class ChannelCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final image = ImageWidget(
      ...
      item.type ?? '',
      item.object?.asset?.channelAvatar?[0] ?? '',
    );
