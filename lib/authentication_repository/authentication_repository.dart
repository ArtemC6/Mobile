...
import 'package:voccent/authentication_repository/cache.dart';
...
class AuthenticationRepository {
  ...
  Future<void> logInWithGoogle() async {
    try {
      ...
      try {
        await linkAccountsOnError(e);
      } on FirebaseAuthException catch (err) {
        throw LogInWithGoogleFailure.fromCode(err.code);
      }
    } catch (_) {
  ...
  AuthProvider getProviderForProviderId(String providerId) {
    ...
    if (providerId == TwitterAuthProvider.PROVIDER_ID) {
      ...
      throw Exception('No provider implemented for $providerId');
    }
  }

  Future<void> linkAccountsOnError(FirebaseAuthException err) async {
    if (err.email != null) {
      final actionCodeSettings = ActionCodeSettings(
        url: 'https://app.voccent.com/',
        handleCodeInApp: true,
        iOSBundleId: 'com.voccent.learn.stg.iPad',
        androidPackageName: 'com.voccent.voccent',
      ...
      try {
        ...
        log('Authentication link sent to ${err.email}');
      } catch (sendLinkErr) {
        log('Failed to send authentication link to ${err.email}: $sendLinkErr');
      }
    } else {
      log(
        'Missing email for auth link; provide an email.',
      );
    }
  }
  ...
  /// [FirebaseUser.empty] from the [user] Stream.
  ///
